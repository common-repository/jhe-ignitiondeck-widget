<?php
/*
Plugin Name: jhe-ignitiondeck-widget
Version: 1
URI: http://websiteservices.shop-from-japan.com/
Description: Add a Widget that display IgnitionDeck projects by selected project-type, this let you have a "featured projects" section.
Author: junh
Author URI: http://websiteservices.shop-from-japan.com/
License: GPL2
*/
...
*/
class Jhe_Fh_Project_Grid_Widget extends WP_Widget {
	/**
	* Register widget with WordPress
	*/
	function __construct() {
		parent::__construct(
			'jhe_fh_project_grid_widget',
			__('JHE 500 Project Grid', 'fivehundred'),
			array('description' => __('A jhe widget to display projects in a grid', 'fivehundred')),
			array('width' => 'auto'));
	}
	/**
	...
	function widget($args, $instance) {
		if (!empty($instance)) {
			...
			// Category
			// JHE replace project_category by project_type
			$tax_slug = $instance['category'];
			if (!empty($tax_slug)) {
				$tax_cat = get_term_by('slug', $tax_slug, 'project_type');
				if (!empty($tax_cat)) {
	...
	function form($instance) {
		...
		// Category field (project type)
		$form .= '<p>';
		$form .= '<label for="'.$this->get_field_id( 'category' ).'">'.__('Project Type (Optional)', 'fivehundred').':';
		...
		$form .= '<div style="font-size: 90%; color: #666; margin-bottom: 10px; margin-top: -10px;">'.__('Enter the slug of the Project Type to only show projects from certain types', 'fivehundred').'</div>';
		$form .= '</label></p>';
		// OrderBy field
		$form .= '<p>';
...
function jhe_load_ignitiondeck_widget() {
    register_widget( 'Jhe_Fh_Project_Grid_Widget' );
}
add_action( 'widgets_init', 'jhe_load_ignitiondeck_widget' );
?>